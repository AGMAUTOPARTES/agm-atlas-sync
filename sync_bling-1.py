...
def sync_produtos(client, since=None, ate=None, modo_execucao="full"):
    ...
    for p in lista:
        ...
        # Imagem do produto: o Bling devolve um campo simples "imagemURL"
        # (url publica, sem criptografia) e, quando a imagem foi enviada
        # dentro do proprio Bling (upload manual), tambem uma midia.imagens
        # mais rica. Preferimos a midia interna quando existe (e a imagem
        # que o Jonas efetivamente cadastrou no Bling); "imagemURL" fica
        # como alternativa. Como sync_produtos sempre roda --full (cadastro
        # inteiro, nunca so o que mudou), esse valor se atualiza sozinho a
        # cada ciclo de sync (~4h via GitHub Actions) sem precisar de
        # nenhuma logica extra de expiracao/revalidacao.
        midia = d.get("midia") or {}
        imagens_internas = (midia.get("imagens") or {}).get("internas") or []
        imagem_interna = imagens_internas[0] if imagens_internas else {}
        imagem_url = (
            (imagem_interna.get("link") if isinstance(imagem_interna, dict) else None)
            or d.get("imagemURL")
            or ""
        ).strip() or None
        rows.append({
